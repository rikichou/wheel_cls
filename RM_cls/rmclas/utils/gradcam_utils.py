...
#from rmclas.modeling import backbones
...
class GradCAM:
    ...
    def __init__(self, model, target_layer_name, colormap='viridis'):
        ...
        self.data_mean = torch.tensor([127.5, 127.5, 127.5])
        #self.data_std = torch.tensor(model.cfg.img_norm_cfg['std'])
        self.data_std = torch.tensor([1, 1, 1])
    ...
    def _register_hooks(self, layer_name):
        ...
        #print('--------------------')
        #print(prev_module._modules['backbone'].layer8.block3.relu)
        # for name, module in prev_module._modules['backbone'].items():
        # #     # x = module(x)
        #     print("name:{}".format(name))
        #for layer in layer_ls:
        #    prev_module = prev_module._modules[layer]

        target_layer = prev_module._modules['backbone'].layer8.block3.relu
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        ...
        '''
        score = torch.max(preds, dim=-1)[0] # add
    ...
    def __call__(self, inputs, use_labels=False, alpha=0.5):
        ...
        blended_imgs = self._alpha_blending(localization_map, inputs,  #inputs['imgs']
                                            alpha)
