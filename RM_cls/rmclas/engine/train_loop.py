...
import rmclas.utils.comm as comm
from rmclas.data.transforms import Cutmix, MixUp, Fmix
from rmclas.utils.events import EventStorage, get_event_storage
...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        """
        if you want to use batch operator, you can change code here
        """
        if self.cfg.INPUT.CUTMIX:
            op = Cutmix(self.cfg.INPUT.MIX_ALPHA)
            data = op(data)
        elif self.cfg.INPUT.MIXUP:
            op = MixUp(self.cfg.INPUT.MIX_ALPHA)
            data = op(data)
        elif self.cfg.INPUT.FMIX:
            op = Fmix(self.cfg.INPUT.MIX_ALPHA)
            data = op(data)
    ...
    def _write_metrics(self, loss_dict: Dict[str, torch.Tensor], data_time: float):
        ...
        with torch.cuda.stream(torch.cuda.Stream() if device.type == "cuda" else None):
            metrics_dict = {k: v.detach().cpu().item() for k, v in loss_dict.items() if torch.is_tensor(v)}
