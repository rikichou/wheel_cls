...
from rmclas.data import build_test_loader, build_train_loader
from rmclas.evaluation import (Evaluator, inference_on_dataset, print_csv_format)
from rmclas.modeling.meta_arch import build_model
from rmclas.solver import build_lr_scheduler, build_optimizer
from rmclas.utils import comm
from rmclas.utils.checkpoint import Checkpointer
from rmclas.utils.collect_env import collect_env_info
from rmclas.utils.env import seed_all_rng
from rmclas.utils.events import CommonMetricPrinter, JSONWriter, TensorboardXWriter
from rmclas.utils.file_io import PathManager
from rmclas.utils.logger import setup_logger
from rmclas.utils.gradcam_utils import GradCAM
from . import hooks
...
def default_argument_parser():
    """
    Create a parser with some common arguments used by rmclas users.
    Returns:
        argparse.ArgumentParser:
    """
    parser = argparse.ArgumentParser(description="rmclas Training")
...
class DefaultPredictor:
    ...
    def __call__(self, image):
        ...
        Returns:
            predictions (torch.tensor): the output predictions of the model
        """
        inputs = {"images": image}
...
class  DefaultTrainer(TrainerBase):
    ...
    It is only guaranteed to work well with the standard models and training workflow in rmclas.
    ...
    def __init__(self, cfg):
        ...
        """
        super().__init__()
        logger = logging.getLogger("rmclas")
        if not logger.isEnabledFor(logging.INFO):  # setup_logger is not called for rmclas
            setup_logger()
    ...
    def build_model(cls, cfg):
        ...
        Returns:
            torch.nn.Module:
        It now calls :func:`rmclas.modeling.build_model`.
    ...
    def build_optimizer(cls, cfg, model):
        ...
        It now calls :func:`rmclas.solver.build_optimizer`.
    ...
    def build_lr_scheduler(cls, cfg, optimizer, iters_per_epoch):
        """
        It now calls :func:`rmclas.solver.build_lr_scheduler`.
    ...
    def build_train_loader(cls, cfg):
        ...
        Returns:
            iterable
        It now calls :func:`rmclas.data.build_detection_train_loader`.
    ...
    def build_test_loader(cls, cfg):
        ...
        Returns:
            iterable
        It now calls :func:`rmclas.data.build_detection_test_loader`.
        Overwrite it if you'd like a different data loader.
        """
        return build_test_loader(cfg, mode='val')

    @classmethod
    def build_evaluator(cls, cfg, output_dir=None):
        data_loader = cls.build_test_loader(cfg)
        return data_loader, Evaluator(cfg, output_dir)
    ...
    def test(cls, cfg, model):
        ...
        data_loader, evaluator = cls.build_evaluator(cfg)
    ...
    def auto_scale_hyperparams(cfg, num_classes):
        ...
        return cfg
    """    
    def activate_gradcam(model, cfg):
        
        Activate the gradcam to visualize the weight learnt by the model during training
        
        model = GradCAM(model, cfg.GRADCAM_TARGETLAYER)
