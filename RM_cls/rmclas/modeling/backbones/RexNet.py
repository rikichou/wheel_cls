...
class ReXNetV1(nn.Module):
    def __init__(self, input_ch=16, final_ch=64, width_mult=1.0, depth_mult=1.0, classes=1000,
                 use_se=True,
                 se_ratio=12,
                 dropout_ratio=0.2,
                 bn_momentum=0.9):
        ...
        '''self.output = nn.Sequential(
            ...
            nn.Conv2d(pen_channels, classes, 2, bias=True))'''
    ...
    def forward(self, x):
        x = self.features(x)
        '''x = self.output(x).flatten(0)
        print(x)'''
        return x

@BACKBONE_REGISTRY.register()
def build_ReXNetV1_backbone(cfg):
    """
    Create a ReXNetV1 instance from config.
    Returns:
        ReXNetV1: a :class:`ReXNetV1` instance.
    """
    # fmt: off
